...
class MIDIListener:
    ...
    def _listen(
        ...
        try:
            ...
            midiin.set_callback(
                MIDIInputHandler(
                    port_name, self._trigger_repository, self._publish_queue
                )
            )
    ...
    def start_thread(self, *, close_event: threading.Event) -> threading.Thread:
        ready_event = threading.Event()
        thread = threading.Thread(target=self._listen, args=(ready_event, close_event))
        thread.daemon = True
        thread.start()
